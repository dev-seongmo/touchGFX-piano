...
play_view_tetrisView::play_view_tetrisView()
{
    std::memset(judgeBuf, 0, sizeof(judgeBuf));
    judge_text.setWildcard(judgeBuf);
}

void play_view_tetrisView::setupScreen()
{
    play_view_tetrisViewBase::setupScreen();

    currentSong = &getTetrisThemeSong();
    nextSongIndex = 0;
    previousInputMask = 0;
    combo = 0;
    finishPending = false;
    ...
    presenter->setSoundProfile(RHYTHM_SOUND_TETRIS);
...
void play_view_tetrisView::tearDownScreen()
{
    presenter->stopSong();
    play_view_tetrisViewBase::tearDownScreen();
}

void play_view_tetrisView::handleTickEvent()
{
    play_view_tetrisViewBase::handleTickEvent();
...
void play_view_tetrisView::checkLaneInput(uint8_t lane)
{
    const uint32_t now = presenter->getSongMs();
    touchgfx::Widget* widgets[MAX_ACTIVE_NOTES] = {
        &note_0, &note_1, &note_2, &note_3, &note_4,
...
void play_view_tetrisView::updateComboText()
{
...
void play_view_tetrisView::updateJudgeText(const char* txt)
{
...
void play_view_tetrisView::setJudgeColor(const char* txt)
{
...
int play_view_tetrisView::getLaneX(uint8_t lane) const
{
